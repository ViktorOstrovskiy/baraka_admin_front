...
import {deletePaymentType, getPaymentType, getPaymentTypeById} from "../action.js";

const PaymentTypeList = ({selectedProfession, setSelectedProfession, step, setStep}) => {
    ...
    const loadProfessionsById = async (name) => {
        const resp = await getPaymentTypeById(pageKeyWords, name);
        if (resp) {
    ...
    const loadData = async () => {
        const resp = await getPaymentType(page, search);
        if (resp) {
    ...
    const handleClickSearch = async () => {
        ...
        setPage(1)
        const resp = await getPaymentType(1, search);
        if (resp) {
            setListProfession(resp);
        }
        ...
        <div className='WorkTypeList'>
            {step === 1 &&
                ...
                <div className="Table-head">
                    <div className="Table-head-item AccountList-item">Payment type Name</div>
