...
import {searchPaymentType} from "../action.js";


const SearchPaymentType = ({handleClickItemSearch}) => {
    ...
    const handleClick = async () => {
        ...
        try {
            const resp = await searchPaymentType(search);
            setNews(resp);
        } catch (err) {
            console.error(err);
        } finally {
        ...
        <div className='SearchProfessions'>
            <div className='SearchProfessions-search-content'>
                <div className='SearchProfessions-search'>
                    <Input placeholder='Enter a query to search for payment type...' onChange={(e) => setSearch(e.target.value)} value={search}/>
...
export default SearchPaymentType;
